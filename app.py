...
def set_sess(url, ses_disp_str):
    ...
    if (len(session["urls"]) == 0) or (
        len(session["urls"]) > 0 and session["urls"][-1] != url
    ):  # consecutive visits to same slide
...
def modify_url_domain(url):
    return url.replace(
        "http://localhost:8096/", "http://timan102.cs.illinois.edu/explanation//"
    )
...
def slide(course_name, lno):
    ...
    (
        ...
        lnos,
        ses_disp_str,
    ) = resolve_slide(course_name, lno, "drop-down")
    vis_urls, vis_strs = get_prev_urls()
    ...
    return render_template(
        ...
        disp_snippet=disp_snippet,
        related_course_names=related_course_names,
        lno=lno,
        lec_name=lec_name,
        lec_names=lec_names,
        lnos=lnos,
...
def related_slide(course_name, slide_name, lno):
    ...
    (
        ...
        lnos,
        ses_disp_str,
    ) = resolve_slide(course_name, lno, "related", slide_name=slide_name)
    vis_urls, vis_strs = get_prev_urls()
...
def next_slide(course_name, lno, curr_slide):
    ...
    (
        ...
        lnos,
        ses_disp_str,
    ) = resolve_slide(course_name, lno, "next", slide_name=curr_slide)
...
def prev_slide(course_name, lno, curr_slide):
    ...
    (
        ...
        lnos,
        ses_disp_str,
    ) = resolve_slide(course_name, lno, "prev", slide_name=curr_slide)
...
def log_helper(action, route):
    if action is not None and route is not None:
        ...
        if func_type == "related_slide":
            ...
            resolve_slide(
                route_ele[beg], route_ele[beg + 1], "drop-down", log=True, action=action
            )
