...
title_tfidfs_path = os.path.join(
    static_path, "tf_idf_outputs", "normalized_title_tfidfs.npy"
)
...
slide_titles = io.open(
    os.path.join(static_path, "slide_titles.txt"), "r", encoding="utf-8"
).readlines()
...
def trim_name(slide_name):
    ...
    for i, n in enumerate(name):
        if (
            ...
            and i > 0
            and name[i - 1].lower() == "part"
        ) or (len(re.findall("[0-9\.]+", n)) == 0):

            if (
                (n == "Lesson")
                or (n in new_name)
                or (len(re.findall("[0-9\.]+", n)) == 0 and len(n) <= 2)
            ):
                continue
            new_name += [n]
...
def get_snippet(slide_name, related_slide_name):
    ...
    tfidf1 = tfidfs[idx1, :]
    tfidf2 = tfidfs[idx2, :]
    term_sims = 2.8956628 * (title_tfidf1 * title_tfidf2) + 5.92724651 * (
        tfidf1 * tfidf2
    )
    ...
    return (
        (
            "Slide title : "
            + title_mapping[related_slide_name][:-1]
            + "\n"
            + "Matching keywords: "
            + keywords
            + "\n"
...
def load_related_slides():
    ...
    for row in related_slides:
        ...
        for col_num in range(1, len(cols), 2):
            ...
            if os.path.exists(
                os.path.join(slides_path, course_name, lec_name, pdf_name)
            ):
...
def get_next_slide(course_name, lno, curr_slide=None):
    ...
    lectures = sort_slide_names(os.listdir(os.path.join(slides_path, course_name)))
    lno = int(lno)
    slides = sort_slide_names(
        os.listdir(os.path.join(slides_path, course_name, lectures[lno]))
    )
    ...
    if len(slides) > 0:
        ...
        if lno == len(lectures) - 1:
            ...
            next_slide = sort_slide_names(
                os.listdir(os.path.join(slides_path, course_name, lectures[lno + 1]))
            )[0]
            lno += 1
...
def get_related_slides(slide_name):
    ...
    if slide_name in related_dict:
        ...
        for r in related_slides:
            ...
            related_slide_name = " ".join(
                comp[-2].replace(".txt", "").replace("_", "-").split("-")
            ).title()
            slide_course_name = " ".join(
                slide_comp[0].replace("_", "-").split("-")
            ).title()
            related_slide_course_name = " ".join(
                comp[0].replace("_", "-").split("-")
            ).title()
            trimmed_name = (
                " ".join(comp[0].replace("_", "-").split("-")).title()
                + " : "
                + trim_name(related_slide_name)
            )
